...
git clone https://github.com/WHEELYDOS/FullStack.git
...
# Run any Flask application
cd 04_flask_crud_app
python app.py

# Or run a Django application
cd ../07_django_blog
...
Static pages can be opened directly in any web browser or launched using a development server such as VS Code's **Live Server**.

For Tailwind projects using the build pipeline:

```bash
cd FullStack/frontend/tailwind/tailwind_starter
npm install
...
- Build artifacts (`node_modules/`, `dist/`, cache files) and virtual environments (`venv/`, `.env`) are excluded via `.gitignore`.
- SQLite database binaries (`*.sqlite3`, `*.db`) are ignored to ensure clean and portable repository state across environments.
- Consistent, lowercase and snake_case naming conventions are maintained across all directories and files.
